...
bool ApplePS2Keyboard::dispatchKeyboardEventWithScancode(UInt8 scanCode)
{
  ...
  else
  {
    ...
    switch (scanCode & ~kSC_UpBit)
    {
      ...
      case 0x17: keyCode = 0x6f; break;        // Brightness up
      case 0x12: keyCode = 0x46; break;        // Brightness down
...
IOReturn ApplePS2Keyboard::message(UInt32 type, IOService* provider, void* argument)
{
    ...
    {
        UInt8 arg = *static_cast<UInt8*>(argument);
        dispatchKeyboardEventWithScancode(arg);
     //   IOLog("Keyboard Event Code %x %d\n",arg,arg);
